...
void RenderTextureFile::render(GLFWwindow * window, std::vector<float> & drawPoints, std::vector<textureRef> & textureCord){
    ...
    for(int i = 0; i < numTriangles; i++){
        ...
        std::vector<float> tri_uv;
        tri1.resize(6);

        // smallest x
        int small_x = drawPoints[startingDraw];
        for(int j = 0; j < 3; j++){
            if(drawPoints[startingDraw + j * 3] < small_x){
                small_x = drawPoints[startingDraw + j * 3];
            }
        }

        // smallest y
        int small_y = drawPoints[startingDraw + 1];
        for(int j = 0; j < 3; j++){
            if(drawPoints[startingDraw + j * 3 + 1] < small_y){
                small_y = drawPoints[startingDraw + j * 3 + 1];
            }
        }

        // largest x
        int large_x = drawPoints[startingDraw];
        for(int j = 0; j < 3; j++){
            if(drawPoints[startingDraw + j * 3] > large_x){
                large_x = drawPoints[startingDraw + j * 3];
            }
        }

        // largest y
        int large_y = drawPoints[startingDraw + 1];
        for(int j = 0; j < 3; j++){
            if(drawPoints[startingDraw + j * 3 + 1] > large_y){
                large_y = drawPoints[startingDraw + j * 3 + 1];
            }
        }

        // size of the triangle
        //int size_x = large_x - small_x;
        //int size_y = large_y - small_y;      
